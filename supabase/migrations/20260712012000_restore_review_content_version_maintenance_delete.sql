-- Anonymous enrichment hardening accidentally removed the maintenance-delete
-- escape hatch used by controlled purges and test-data cleanup. Keep the
-- append-only default while restoring explicit transaction-local maintenance.
...
begin
  ...
  if old.changed_by is not null
    or old.changed_by_role is not null
    or old.change_source is distinct from 'database_trigger'
    or old.metadata ? 'enrichedBy'
    or coalesce(old.metadata->>'source', 'database_trigger') is distinct from 'database_trigger'
  then
...
revoke all privileges on function public.prevent_review_content_version_mutation()
