        ...
        <div className="mt-4 grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div>
            <h1 className="max-w-4xl text-2xl font-black leading-tight text-slate-950 sm:text-3xl">
              채널별 운영 폴더의 마감 준비와 증빙 리스크를 검토합니다.
            </h1>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600">
              관리자는 템플릿을 강제하기보다 각 채널이 직접 구성한 예산,
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[920px] text-left text-sm">
            <thead className="bg-slate-50 text-xs font-black uppercase tracking-[0.08em] text-slate-500">
              <tr>
                <th className="px-5 py-3">채널 / 폴더</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {rows.map(({ project, summary }) => (
                <tr key={project.id}>
                  ...
                  <td className="px-5 py-4">
                    <Link
                      className="inline-flex min-h-11 items-center gap-2 rounded-md border border-slate-200 px-3 text-xs font-black text-slate-700 hover:border-[var(--primary)] hover:text-[var(--primary)]"
