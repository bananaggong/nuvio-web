...
type DynamicAnswers = Record<string, string | boolean>;
...
export function ProgramApplicationForm({
  ...
  const hasTemplate = Boolean(formTemplate && formTemplate.fields.length > 0);
  ...
  function updateDynamicAnswer(fieldId: string, value: string | boolean) {
  ...
  function buildAnswers(): Record<string, unknown> {
    if (formTemplate && hasTemplate) {
      return {
        ...
        templateAnswers: formTemplate.fields.map((field) => ({
          id: field.id,
          label: field.label,
          type: field.type,
              ...
              <div className="mt-4 grid gap-4">
                {formTemplate.fields.map((field) => (
                  <DynamicField
                    field={field}
                    key={field.id}
                    onChange={(value) => updateDynamicAnswer(field.id, value)}
