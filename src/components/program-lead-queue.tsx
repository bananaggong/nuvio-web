...
import { Check, ExternalLink, Plus, Radar, RotateCw } from "lucide-react";
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div className="flex min-w-fit gap-2">
                  <button
                    ...
                    disabled={savedLeadIds.has(lead.id)}
                    onClick={() => saveDraft(lead)}
                    type="button"
                  >
                    {savedLeadIds.has(lead.id) ? <Check size={13} /> : <Plus size={13} />}
                    {savedLeadIds.has(lead.id) ? "저장됨" : "초안 저장"}
                  </button>
                  {lead.sourceUrl ? (
                    <Link
