...
- 최종 문서/출시 판정: 완료, production migration 적용 및 배포 후 smoke test 대기
...
- 배포 후 runtime blocker 2개(RR-001, RR-002)
...
- production 능동 테스트는 범위상 금지

## 다음 작업

1. 독립 검토 결과의 release-blocking finding을 반영한다.
2. 최종 lint/typecheck/security/build/diff 검증을 재실행한다.
3. migration/staging runbook과 잔여 위험을 최종 확정한다.
4. push/merge/deploy 없이 사용자에게 결과를 전달한다.
