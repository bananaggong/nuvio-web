...
| Layer 3. NUVIO Ops | NUVIO 관리자 | 소식지를 작성하고 공개 콘텐츠를 관리한다. |
...
| `/admin` | `/admin/magazine`으로 이동 |
| `/admin/magazine` | 소식지 관리 |
| `/admin/magazine/new` | 새 소식지 작성 |
| `/admin/magazine/[id]/edit` | 소식지 수정 |
...
공개 공지는 NUVIO가 직접 관리하는 내부 공지만 노출합니다. 공개 프로그램은 DB 게시 데이터와 개발 모드에서만 사용하는 seed 데이터로 구성합니다.
...
# Optional: enable only after the Supabase custom provider is configured.
...
WEB_PUSH_VAPID_SUBJECT=mailto:support@nuvio.kr
META_APP_ID=
META_APP_SECRET=
